...
const MyAccount = () => {
  ...
  const [userInfoFromContext, setUserInfoFromContext] = useState([])

  const { username } = useParams()
  
  const editAccount = () => {
    navigate(`/edit-account/${userInfoFromContext.username}`)
  }

  const getUserDataFromContext = useCallback(() => {
    getUserByParams({ email: user.email })
    .then(response => setUserInfoFromContext(response[0]))
  }, [])
